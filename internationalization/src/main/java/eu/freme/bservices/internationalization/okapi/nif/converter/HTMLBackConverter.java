...
public class HTMLBackConverter {
	...
	public InputStream convertBack(String nifVersion,
			final InputStream skeletonFile, final InputStream enrichedFile) {
	...
	public InputStream convertBack(String nifVersion,
			final InputStream skeletonFile, final InputStream enrichedFile,
			final String skeletonFormat, final String enrichedFormat) {
	...
	private String convertBack(String nifVersion) {
		...
		if (tbp != null) {
			...
			// Handling its-annotators-ref attribute in HTML (only when
			// nifVersion 2.1)
			if (isNif21) {
				...
				if (annotatorsRefStmts != null && !annotatorsRefStmts.isEmpty()) {
					...
					if (!HTMLBackConverterHelper.isHtmlSnippet(skeletonContext)) {
						...
						String attributeValue = statement.getObject()
								.asResource().getURI();
						logger.debug("Found Tool Annotation");
						Document doc = Jsoup.parse(skeletonContext);
						String attributeName = HTMLBackConverterHelper
								.getAttributeNameForItsAnnot(RDFConstants.TA_ANNOTATORS_REF);
						if (doc.select("html").hasAttr(attributeName)) {
							String oldValue = doc.select("html").attr(
									attributeName);
							if (!HTMLBackConverterHelper
									.isThisToolAnnotationPresent(oldValue,
											attributeValue)) {
								if (oldValue != null && !oldValue.equals("")) {
									String updatedValue = oldValue + " "
											+ attributeValue;
									doc.select("html")
	...
	/**
	 * Builds a map between an enriched text and the list of html target
	 * elements
	 * 
	 * @param tuResources
	 *            the text unit resources list
	...
	private Map<String, List<String>> buildAnnotationsMap(
		...
		for (TextUnitResource tur : tuResources) {
			...
			if (!entityStmts.isEmpty()) {
				...
				annotatedText.append(HTMLBackConverterHelper
						.buildAnnotsAttributesString(entityStmts, null));
		...
		logger.debug("\nEnrichments:\n");
		enrichments.forEach((k, v) -> logger.debug(k + "=" + v + ",size="
				+ v.size()));
		return enrichments;
	}
	...
	private List<Statement> findTermStmts(TextUnitResource resource) {
		...
		Property termInfoRefProp = model.createProperty(
				RDFConstants.itsrdfPrefix, ItsRdfConstants.TERM_INFO_REF);

		StmtIterator stmtsWithTermProp = model.listStatements(
				resource.getResource(), termProp, (RDFNode) null);
		StmtIterator stmtsWithTermInfoRefProp = model.listStatements(
				resource.getResource(), termInfoRefProp, (RDFNode) null);
	...
	private List<Statement> findAnnotatorsRefStmts() {
		...
		Property annotatorsRefProp = model.createProperty(
				RDFConstants.itsrdfPrefix, ItsRdfConstants.TA_ANNOTS_REF);

		StmtIterator stmtsWithAnnotatorsRefProp = model.listStatements(null,
				annotatorsRefProp, (RDFNode) null);

		while (stmtsWithAnnotatorsRefProp != null
				&& stmtsWithAnnotatorsRefProp.hasNext()) {
	...
	private List<TextUnitResource> listTextUnitResources(String nifVersion) {
		...
		Property anchorOfProp = model.createProperty(nifPrefix
				+ RDFConstants.ANCHOR_OF);
		...
		while (anchorStmts.hasNext()) {
			...
			unitRes = new TextUnitResource(anchorStmt.getSubject(), anchorStmt
					.getObject().asLiteral().getString(), nifVersion);
	...
	private String findSkeletonContextString(String nifVersion) {
		...
		boolean isNif20 = StringUtils.isEmpty(nifVersion)
				|| nifVersion.equals("2.0");

		String offsetPrefix = isNif20 ? RDFConstants.NIF20_OFFSET
				: RDFConstants.NIF21_OFFSET;

		Property wasConvertedFromProp = model.createProperty(nifPrefix
				+ RDFConstants.WAS_CONVERTED_FROM);
		...
		if (wasConvFromNodes != null && wasConvFromNodes.hasNext()) {
			...
			Property isStringProp = model.createProperty(nifPrefix
					+ RDFConstants.IS_STRING);
	...
	class TextUnitResource {
		...
		public TextUnitResource(Resource resource, String text,
			...
			String[] offset = resource.getURI()
					.substring(offsetIdx + offsetPrefix.length())
					.split(splitter);
